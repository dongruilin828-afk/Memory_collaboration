...
抓取完成后继续调用已配置的总结后端：
    ...
    uv run parser.py --summarize

临时覆盖集成流程使用的模型：

    uv run parser.py --summarize --model gemini-3.6-flash

对已经导出的结果单独总结，不重新打开浏览器：

    uv run summarize_memory.py "results/ChatGPT_含图片.md"

临时选择其他模型：

    uv run summarize_memory.py "results/ChatGPT_含图片.md" --model gemini-3.6-flash
...
显式使用 SiliconFlow 的 Qwen 备用模型：
    ...
    uv run summarize_memory.py "results/DeepSeek_超长对话.md" --provider siliconflow --model Qwen/Qwen3.5-397B-A17B
...
输出统一保存在 summary 文件夹，并使用源对话文件名区分：

- summary/ChatGPT_含图片_result.json：结构化结果，适合后续程序读取。
- summary/ChatGPT_含图片_summary.md：适合直接阅读和人工检查。
...
保存到 summary_detailed，普通精简版继续保存在 summary。JSON 始终保留
完整结构化数据。
...
对 results 中 tests.txt 对应的全部现有结果直接批量总结，不重新抓取网页：

    uv run summarize_tests.py
...
重新抓取 tests.txt 中的全部链接，并在抓取完成后批量总结：
