...
在 GUI 中粘贴分享链接、勾选一个或多个模式并选择保存目录。普通版或详细版
完成主题综合后，会弹出“选择重点板块”窗口，只列出本次确实有内容的分类；
勾选项只控制分主题摘要后的同级展开板块，完整主题、媒体开发检查以及详细版
细节记忆不受影响。仅抓取和极简版不会弹出该窗口。
